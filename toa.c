/*
 * Fork From dpvs toa
 * Modify: ipv4 ipv6 option, capable with with kernel-2.6.32 and older
...
unsigned long (*kallsyms_lookup_name_f)(char *name) = 0;

static bool module_load_success = false;
        ...
        while (length > 0) {
            ...
            default:
                ...
                if (TCPOPT_TOA == opcode &&
                    ...
                    TOA_DBG("af = %d, find v4 toa data: ip = "
                        TOA_NIPQUAD_FMT", port = %u\n",
                        af,
                        TOA_NIPQUAD(tdata.ip),
                        ntohs(tdata.port));
                    if (af == AF_INET) {    // if socket is v4, v4 client -> v4 server
                        memcpy(&ret_ptr, &tdata,
                            sizeof(ret_ptr));
                        TOA_DBG("find toa_v4 data in v4 socket: %p\n",
                            ret_ptr);
                        return ret_ptr;
                    }
#ifdef TOA_IPV6_ENABLE
                    else if (af == AF_INET6) {  // if socket is v6, v4 client -> v6 server
                        ...
                        ptr_toa_ip6->opcode = TCPOPT_TOA_IPV6;  // 要改为v6的code
                        ...
                        //     ptr_toa_ip6);
                        TOA_DBG("find toa_v4 data in v6 socket : ip = "
                            TOA_NIP6_FMT", port = %u,"
                            " coded ip6 toa data: %p\n",
                            TOA_NIP6(ptr_toa_ip6->in6_addr),
                            ntohs(ptr_toa_ip6->port),
                            ptr_toa_ip6);
                ...
                if (TCPOPT_TOA_IPV6 == opcode &&
                    ...
                    TOA_DBG("find toa_v6 data in v4/v6 socket: ip = "
                        TOA_NIP6_FMT", port = %u,"
                        " coded ip6 toa data: %p\n",
                        TOA_NIP6(ptr_toa_ip6->in6_addr),
                        ntohs(ptr_toa_ip6->port),
                    ...
                    TOA_INC_STATS(ext_stats, IP6_ADDR_ALLOC_CNT);
                    if (af == AF_INET6)
                        *nat64 = 0;     // v6 client -> v6 server
                    else
                        *nat64 = 1;     // v6 client -> v4 server
...
#endif
{
    ...
    if (retval >= 0 && NULL != sk->sk_user_data && peer) {
        ...
        if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready) {
            if (!sock_flag(sk, SOCK_NAT64)) {   //  4 -> 4
            ...
            }else if (sock_flag(sk, SOCK_NAT64)) {  // 6 -> 4
                lock_cpu_toa_ip6_sk();
                struct sockaddr_in *sin4 = (struct sockaddr_in *) uaddr;
                ...
                if (sk == ptr_ip6_entry->sk &&
                    TCPOPT_TOA_IPV6 == ptr_ip6_data->opcode &&
                    TCPOLEN_IP6_TOA == ptr_ip6_data->opsize) {
                    TOA_INC_STATS(ext_stats, GETNAME_TOA_OK_CNT);
                    TOA_DBG("inet_getname_toa: set new sockaddr, ip "
                        TOA_NIPQUAD_FMT" -> "TOA_NIP6_FMT
                        ", port %u -> %u\n",
                        TOA_NIPQUAD(sin4->sin_addr.s_addr),
                        TOA_NIP6(ptr_ip6_data->in6_addr),
                        ntohs(sin4->sin_port),
                        ntohs(ptr_ip6_data->port));
                    sin6->sin6_port = ptr_ip6_data->port;
                    sin6->sin6_addr = ptr_ip6_data->in6_addr;
                    sin6->sin6_family = AF_INET6;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,1)
                    retval = sizeof(struct sockaddr_in6);
#else
...
inet64_getname_toa(struct sock *sk, int cmd, void __user *user, int *len)
{
    ...
    if (NULL != sk->sk_user_data) {
        ...
        if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready) {
            ...
            if (TCPOPT_TOA_IPV6 == ptr_ip6_data->opcode &&
                TCPOLEN_IP6_TOA == ptr_ip6_data->opsize) {
                TOA_INC_STATS(ext_stats, GETNAME_TOA_OK_CNT);
                ...
                *len = sizeof(struct toa_nat64_peer);
                ...
                TOA_DBG("inet64_getname_toa, toa mismatch, opcode:%d, opsize:%d\n",
                    ptr_ip6_data->opcode, ptr_ip6_data->opsize);
                TOA_INC_STATS(ext_stats,
                        GETNAME_TOA_MISMATCH_CNT);
            }
        } else {
...
{
        inet6_stream_ops_p =
                (struct proto_ops *)kallsyms_lookup_name_f("inet6_stream_ops");
        ...
        ipv6_specific_p =
                (struct inet_connection_sock_af_ops *)kallsyms_lookup_name_f("ipv6_specific");
        ...
        tcp_v6_syn_recv_sock_org_pt =
                (syn_recv_sock_func_pt)kallsyms_lookup_name_f("tcp_v6_syn_recv_sock");
...
static struct proc_ops toa_stats_fops = {
 ...
 */
 ...
 static struct proc_dir_entry *proc_net_fops_create(struct net *net,
    const char *name, mode_t mode, const struct proc_ops *fops)
{
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,33)
/* module init */
static int find_fn_kallsyms_lookup_name (void *data, const char *name, struct module *mod, unsigned long addr)
{ 
	if (name != NULL && strcmp (name, "kallsyms_lookup_name") == 0) {
		kallsyms_lookup_name_f = (unsigned long (*)(char *name))addr;
		return 1;
	}
...
/*
* kallsyms_lookup_name is not exported in kernel version <= 2.6.33, but kallsyms_on_each_symbol is exported!
* Try not to modify kernel src but get kallsyms_lookup_name function address by kallsyms_on_each_symbol in (version <= 2.6.33)
* Also, modify kernel src (in kallsyms.c) to export kallsyms_lookup_name by EXPORT_SYMBOL_GPL is also a way, 
* recomplie kernel is necesarry in that way.
*/
static void get_fn_addr (void)
{ 
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,33)
	kallsyms_on_each_symbol(find_fn_kallsyms_lookup_name, 0);
#elif LINUX_VERSION_CODE <= KERNEL_VERSION(5,7,0)
    //definition of kallsyms_lookup_name as kallsyms.c: unsigned long kallsyms_lookup_name(const char *name)
    kallsyms_lookup_name_f =(unsigned long (*)(char *name))kallsyms_lookup_name;
#else
#ifdef HAVE_KPROBES     // 使用KPROBES, 但要求CONFIG_KPROBES开启
    TOA_INFO("For Linux v5.7+, CONFIG_KPROBES is on, use Kprobes.\n");
    struct kprobe kp = {
        .symbol_name = "kallsyms_lookup_name",
    };
    if (register_kprobe(&kp) < 0)
        return;
    kallsyms_lookup_name_f = (unsigned long (*)(char *name))kp.addr;
...
toa_init(void)
{
    TOA_INFO("TOA " TOA_VERSION " support ipv4 and ipv6.\n");
    ...
    get_fn_addr();  // try to get kallsyms_lookup_name directly first, if not, use param
    if (0 == kallsyms_lookup_name_f) {
        TOA_INFO("CPU [%u] get kallsyms_lookup_name addr fail\n", smp_processor_id());
#ifdef HAVE_PARAM
        TOA_INFO("For Linux v5.7+, CONFIG_KPROBES is off, use param.\n");
        const char sct_name[] = "kallsyms_lookup_name";
        char symbol[40] = { 0 };

        if (sym == 0) {
            TOA_INFO("For Linux v5.7+, Kprobes is the preferable way to get "
                    "symbol.\n");
            TOA_INFO("If Kprobes is absent, you have to specify the address of "
                    "kallsyms_lookup_name symbol\n");
            TOA_INFO("by /proc/kallsyms, which contains all the "
                    "symbol addresses, into sym parameter.\n");
            goto err;
        }
        sprint_symbol(symbol, sym);
        if (!strncmp(sct_name, symbol, sizeof(sct_name) - 1))
            kallsyms_lookup_name_f = (long unsigned int (*)(char *))sym;
        else
            goto err;
#endif
    }

    TOA_INFO("CPU [%u] kallsyms_lookup_name_f = <%p>\n", smp_processor_id(), kallsyms_lookup_name_f);

    /* get the address of function sock_def_readable
    * so later we can know whether the sock is for rpc, tux or others
    */
    sk_data_ready_addr = kallsyms_lookup_name_f("sock_def_readable");
    TOA_INFO("CPU [%u] sk_data_ready_addr = "
        "kallsyms_lookup_name_f(sock_def_readable) = %lu\n",
